...
def simulate_drift() -> DriftReport:
    ...
    history = [
        RunSnapshot(
            run_id=f"healthy-{i}",
            timestamp=f"2026-03-{20 + i}T00:00:00",
            total_tasks=100,
            done_count=95,
            failed_count=5,
...
def simulate_fingerprint_divergence() -> float:
    ...
    baseline = BehavioralFingerprint(
        run_id="baseline",
        dimensions={
            "action_distribution": 0.85,
            "output_structure": 0.90,
            "token_profile": 0.45,
            "verification_pattern": 0.12,
            "tool_selection": 0.78,
            "latency_profile": 0.92,
...
def run_demo() -> None:
    ...
    if report.signals:
        ...
        for s in report.signals:
            print(
                f"  {s.metric:<30s} {s.baseline_mean:>10.4f} {s.current_value:>10.4f} {s.direction:<12s}"
            )
