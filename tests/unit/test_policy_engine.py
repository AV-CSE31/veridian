...
class TestPolicyCompiler:
    def test_compile_yaml_returns_verifier_class(
    ...
    def test_compiled_verifier_blocks_pii(self, task: Task, pii_result: TaskResult) -> None:
        compiler = PolicyCompiler()
        verifier_cls = compiler.compile_yaml(SIMPLE_POLICY_YAML)
        verifier = verifier_cls()
    ...
    def test_compile_field_equals_operator(self, task: Task, clean_result: TaskResult) -> None:
        compiler = PolicyCompiler()
...
class TestBuiltinPolicies:
    ...
    def test_all_builtin_policies_run_without_error(
        self, task: Task, clean_result: TaskResult
    ) -> None:
        compiler = PolicyCompiler()
        for policy in BUILTIN_POLICIES:
    ...
    def test_builtin_policy_frameworks_covered(self) -> None:
        ...
        assert required.issubset(frameworks), f"Missing frameworks: {required - frameworks}"
