...
if TYPE_CHECKING:
    from veridian.observability.ingest import IngestPipeline
...
class VeridianTracer:
    ...
    def _append_event(self, event: TraceEvent) -> None:
        ...
        If an ingest pipeline is configured, route the event through that
        pipeline instead of direct JSONL append.
        """
        event_dict = event.to_dict()
        if self._ingest_pipeline is not None:
            self._ingest_pipeline.emit(event_dict)
            return
        ...
        with self._lock:
            ...
            try:
                ...
                with contextlib.suppress(OSError):
                    os.unlink(tmp)
                # Fallback path: if atomic replace fails (e.g., mocked os.replace
                # in tests or transient filesystem edge-cases), attempt a direct
                # append so critical audit events are not dropped.
                try:
