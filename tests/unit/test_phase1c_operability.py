...
from chit.core.config import ChitConfig, default_data_dir
from chit.ledger.ledger import TaskLedger


class TestDefaultDataDir:
    def test_env_var_creates_and_returns_dir(self, tmp_path: Path) -> None:
        target = tmp_path / "varlib" / "chit"
        with patch.dict(os.environ, {"CHIT_DATA_DIR": str(target)}, clear=False):
