...
from veridian.cli.dlq import dlq_app
from veridian.cli.replay import replay_app
...
# Phase C: DLQ CLI for abandoned task triage
...
def init(
    ...
    # newly initialized projects are immediately compatible with all CLI
    # commands and runner paths.
    TaskLedger(path=p, progress_file=str(p.parent / "progress.md"))
...
def run(
    ...
    share_report: bool = typer.Option(
        False,
        "--share-report",
        help="Write a shareable Markdown report with a Veridian badge after the run.",
    ),
) -> None:
    ...
    if share_report:
        try:
            report_path = generate_share_report(ledger=led, summary=summary)
        except DashboardError as exc:
            console.print(f"\n[yellow]Share report unavailable:[/yellow] {exc}")
        else:
            console.print(f"\n[green]Share report ready:[/green] {report_path}")
