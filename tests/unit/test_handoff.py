...
class TestHandoffPacket:
    def test_packet_has_unique_id(self, task: Task, result: TaskResult, passing_protocol: HandoffProtocol) -> None:
    ...
    def test_packet_contains_task_state(self, task: Task, result: TaskResult, passing_protocol: HandoffProtocol) -> None:
    ...
    def test_packet_contains_verification_history(self, task: Task, result: TaskResult, passing_protocol: HandoffProtocol) -> None:
        packet = passing_protocol.create_packet(task, result, source_agent_id="agent-A")
        assert isinstance(packet.verification_history, list)
        assert len(packet.verification_history) >= 1  # verification ran

    def test_packet_has_hmac_token(self, task: Task, result: TaskResult, passing_protocol: HandoffProtocol) -> None:
        packet = passing_protocol.create_packet(task, result, source_agent_id="agent-A")
        assert packet.hmac_token
        assert len(packet.hmac_token) == 64  # SHA-256 hex

    def test_packet_has_context_summary(self, task: Task, result: TaskResult, passing_protocol: HandoffProtocol) -> None:
    ...
    def test_packet_has_constraints(self, task: Task, result: TaskResult, passing_protocol: HandoffProtocol) -> None:
        packet = passing_protocol.create_packet(
            task, result,
            source_agent_id="agent-A",
    ...
    def test_packet_serializes_to_json(self, task: Task, result: TaskResult, passing_protocol: HandoffProtocol) -> None:
        packet = passing_protocol.create_packet(task, result, source_agent_id="agent-A")
...
class TestHandoffVerificationBoundary:
    ...
    def test_unverified_result_blocked(
        self, task: Task, secret_key: bytes
    ) -> None:
