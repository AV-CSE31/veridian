...
def dashboard_with_records() -> ComplianceDashboard:
    ...
    for i in range(7):
        dash.add_record(
            _rec(task_id=f"t{i}", verifier_id="schema", agent_id="agent-A", passed=True)
        )
    dash.add_record(
        _rec(
            task_id="t7",
    ...
    dash.add_record(
        _rec(
            task_id="t8",
            verifier_id="tool_safety",
            agent_id="agent-B",
            passed=False,
            error="shell=True",
            failure_category="tool_safety",
        )
    )
    dash.add_record(_rec(task_id="t9", verifier_id="tool_safety", agent_id="agent-B", passed=True))
    return dash
...
class TestVerifierStats:
    ...
    def test_verifier_stats_has_required_fields(
    ...
    def test_verifier_stats_failure_count(
        self, dashboard_with_records: ComplianceDashboard
    ) -> None:
        stats = dashboard_with_records.per_verifier_stats()
...
class TestAgentStats:
    ...
    def test_agent_stats_has_required_fields(
        self, dashboard_with_records: ComplianceDashboard
    ) -> None:
        stats = dashboard_with_records.per_agent_stats()
        s = stats["agent-A"]
...
class TestSummaryReport:
    def test_summary_report_is_non_empty_string(
        self, dashboard_with_records: ComplianceDashboard
    ) -> None:
    ...
    def test_summary_report_contains_pass_rate(
        self, dashboard_with_records: ComplianceDashboard
    ) -> None:
        report = dashboard_with_records.generate_summary_report()
