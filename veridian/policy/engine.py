...
class PolicyEngine:
    ...
    def load_policy(self, policy: Policy) -> None:
        """Register a Policy model object directly."""
        verifier_cls = self._compiler.compile(policy)
        self._registry[policy.policy_id] = (policy, verifier_cls)
    ...
    def evaluate(
        ...
        if entry is None:
            raise PolicyNotFound(
                ...
                f"Registered: {self.list_policies()}"
            )
    ...
    def evaluate_all(
        ...
        """
        return {
            policy_id: self.evaluate(policy_id, task, result)
            for policy_id in self.list_policies()
        }

    def policy_metadata(self) -> list[dict[str, Any]]:
        """Return [{policy_id, version, description, framework, content_hash}] for all policies."""
        return [
            {
            ...
            for policy, _ in self._registry.values()
        ]
