...
class TestVerifierPerformance:
    def _make_perf(
        self, tp: int = 0, fp: int = 0, tn: int = 0, fn: int = 0
    ) -> VerifierPerformance:
        return VerifierPerformance(
            verifier_id="v1", true_positives=tp, false_positives=fp,
    ...
    def test_auto_tune_adjusts_sensitivity(self, tmp_path: Path) -> None:
        ...
        for i in range(5):
            store.add(FeedbackRecord(
                "threshold_verifier", f"t{i}",
                verifier_passed=True, human_expected_pass=False,
            ))
