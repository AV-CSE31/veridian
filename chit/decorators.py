...
from chit.core.report import VerificationReport, append_report_jsonl
...
def verified(
    ...
    *,
    ...
    """Verify a function's return value with a Chit verifier.
    ...
    if report_file is not None:
        if report_signing_key is None:
            raise ChitConfigError("report_signing_key is required when report_file is configured")
        key_bytes = (
        ...
        if len(key_bytes) < 32:
            raise ChitConfigError("report_signing_key must contain at least 32 bytes")
        if not report_signing_key_id:
            raise ChitConfigError("report_signing_key_id must be non-empty")
