...
- Optionally uses ``sentence-transformers`` when explicitly enabled via
  ``VERIDIAN_USE_SENTENCE_TRANSFORMERS=1`` (higher quality embeddings)
...
_ST_UTIL: Any | None = None
_ST_UNAVAILABLE = False


def _sentence_transformers_enabled() -> bool:
    raw = os.getenv("VERIDIAN_USE_SENTENCE_TRANSFORMERS", "")
    return raw.strip().lower() in {"1", "true", "yes", "on"}


def _get_sentence_transformers() -> tuple[Any | None, Any | None]:
    """Lazily load and cache sentence-transformers objects."""
    global _ST_MODEL, _ST_UNAVAILABLE, _ST_UTIL
    if _ST_UNAVAILABLE:
        return None, None
...
def _compute_similarity(context: str, output: str) -> float:
    ...
    Uses ``sentence-transformers`` only when explicitly enabled via
    ``VERIDIAN_USE_SENTENCE_TRANSFORMERS=1``. Otherwise it runs the
    deterministic stdlib TF-IDF cosine fallback.
    """
    if _sentence_transformers_enabled():
        model, util = _get_sentence_transformers()
...
class EmbeddingGroundingVerifier(BaseVerifier):
    ...
    - No LLM calls — uses TF-IDF (stdlib) by default, or
      ``sentence-transformers`` when ``VERIDIAN_USE_SENTENCE_TRANSFORMERS=1``.
