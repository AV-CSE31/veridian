...
Loop contract:
  ...
  - Result regex: <chit:result>\\s*(\\{.*?\\})\\s*</chit:result>  (DOTALL)
  - Never hardcode max_turns --- always read from ChitConfig
"""
...
from chit.core.task import Task, TaskResult, TraceStep
from chit.providers.base import LLMProvider, Message
...
_RESULT_RE = re.compile(
    r"<chit:result>\s*(\{.*?\})\s*</chit:result>",
    re.DOTALL,
)

_BASH_RE = re.compile(r"```bash|<bash>|\$ ")

_NUDGE_MESSAGE = "Output a <chit:result> block now."
...
class WorkerAgent:
    ...
    def _build_initial_messages(
        ...
        content = (
            ...
            f'<chit:result>{{"summary": "...", "structured": {{}}}}</chit:result>'
        )
        if attempt > 0 and task.last_error:
            content += f"\n\n[RETRY ERROR]\n{str(task.last_error)[:300]}"

        return [
            {"role": "system", "content": "You are a Chit AI agent."},
