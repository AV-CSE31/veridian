...
from chit.core.exceptions import ChitConfigError, ChitError, VerifierNotFound
...
class BaseVerifier(ABC):
    ...
    def as_guardrail(self) -> Callable[[Any], tuple[bool, Any]]:
        ...
        Chit owning the execution loop.
...
class VerifierRegistry:
    ...
    def _autodiscover(self) -> None:
        ...
        try:
            ...
            eps = importlib.metadata.entry_points(group="chit.verifiers")
            for ep in eps:
                try:
                    cls = ep.load()
                    self.register(cls)
...
registry.register_lazy_many(
    {
        "any_of": "chit.verify.builtin.any_of:AnyOfVerifier",
        "bash_exit": "chit.verify.builtin.bash:BashExitCodeVerifier",
        "composite": "chit.verify.builtin.composite:CompositeVerifier",
        "file_exists": "chit.verify.builtin.file_exists:FileExistsVerifier",
        "http_status": "chit.verify.builtin.http:HttpStatusVerifier",
        "quote_match": "chit.verify.builtin.quote:QuoteMatchVerifier",
        "repo_guard": "chit.verify.builtin.repo_guard:RepoGuardVerifier",
        "schema": "chit.verify.builtin.schema:SchemaVerifier",
    }
)
