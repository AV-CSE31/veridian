...
class LocalJSONStorage(BaseStorage):
    ...
    def get(self, task_id: str, *, tenant_id: str | None = None) -> Task:
        ...
        (its ID must start with ``<tenant_id>::``).  Raises
        ``TenantIsolationError`` otherwise.
        """
        with self._lock():
            data = self._load_raw()
            tasks = self._task_map(data)
            ...
            if tenant_id is not None and not task.id.startswith(f"{tenant_id}::"):
                raise TenantIsolationError(
                    f"Task {task_id!r} does not belong to tenant {tenant_id!r}"
                )
            return task
    ...
    def list_all(self, *, tenant_id: str | None = None) -> list[Task]:
        ...
        ``<tenant_id>::`` are returned.
        """
        with self._lock():
            data = self._load_raw()
            tasks = [Task.from_dict(t) for t in self._task_map(data).values()]
