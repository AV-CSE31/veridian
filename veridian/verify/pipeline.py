...
class PipelineResult:
    ...
    def passed(self) -> bool:
        return all(sr.result is None or sr.skipped or sr.result.passed for sr in self.stage_results)

    @property
    def total_duration_ms(self) -> float:
        return sum(sr.duration_ms for sr in self.stage_results if not sr.skipped)
...
class VerificationPipeline:
    ...
    def from_config(cls, config_dict: dict[str, Any]) -> VerificationPipeline:
        ...
        for stage_cfg in config_dict.get("stages", []):
            ...
            if not verifier_id:
                raise PipelineError(
                    f"Pipeline stage '{stage_cfg.get('name', '?')}' is missing 'verifier_id'."
                )
    ...
    def _run_stage(self, stage: PipelineStage, task: Task, result: TaskResult) -> StageResult:
    ...
    async def run_async(self, task: Task, result: TaskResult) -> PipelineResult:
        ...
        for stage in self._stages:
            sr = await loop.run_in_executor(None, self._run_stage, stage, task, result)
            stage_results.append(sr)
    ...
    async def stream(self, task: Task, result: TaskResult) -> AsyncGenerator[StageResult, None]:
        """
