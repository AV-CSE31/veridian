    ...
    def verify(self, task: Task, result: TaskResult) -> VerificationResult:
        ...
        if risk not in _VALID_RISKS:
            return VerificationResult(
                passed=False, error=f"Invalid risk '{risk}', must be: {sorted(_VALID_RISKS)}"
            )
        if action not in _VALID_ACTIONS:
            return VerificationResult(
                passed=False, error=f"Invalid action '{action}', must be: {sorted(_VALID_ACTIONS)}"
            )
    ...
    cases: list[tuple[str, dict[str, str], str]] = [
        (
            "valid_low_clear",
            {
                "risk_level": "LOW",
                "action": "CLEAR",
                "justification": "Normal pattern",
                "regulation_cited": "AML-2024",
            },
            "Correctly cleared",
        ),
        (
            "valid_critical_block",
            {
                "risk_level": "CRITICAL",
                "action": "BLOCK",
                "justification": "OFAC sanctions match",
                "regulation_cited": "OFAC-SDN",
            },
            "Correctly blocked",
        ),
        (
            "cascade_bug_low_block",
            {
                "risk_level": "LOW",
                "action": "BLOCK",
                "justification": "Low risk but blocking anyway",
                "regulation_cited": "AML",
            },
            "LOW risk + BLOCK = contradiction",
        ),
        (
            "cascade_bug_critical_clear",
            {
                "risk_level": "CRITICAL",
                "action": "CLEAR",
                "justification": "Sanctions match but clearing",
                "regulation_cited": "OFAC",
            },
            "CRITICAL + CLEAR = catastrophic",
        ),
        (
            "missing_justification",
            {"risk_level": "HIGH", "action": "ESCALATE"},
            "No justification = non-compliant",
        ),
        (
            "invalid_risk",
            {
