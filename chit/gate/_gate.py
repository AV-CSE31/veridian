...
produces is a normal ``chit.assurance`` / ``chit.effects`` value that the
...
CONTRACT_SCHEMA_ID = "chit.gate.contract.v1"
STATE_SCHEMA_ID = "chit.gate.state.v1"
...
class Verdict:
    ...
    def failed_clauses(self) -> tuple[ClauseResultV1, ...]:
        ...
        from chit.assurance import ClauseStatus
...
class _FunctionAdapter:
    ...
    def dispatch(self, request: DispatchRequest) -> DispatchResult:
        ...
        payload = encode_profile_v1(
            {
                "schema_id": "chit.gate.dispatch-result.v1",
...
class Gate:
    ...
    principal, one purpose and one ordered set of :class:`~chit.gate.Check`
    ...
    :class:`~chit.assurance.Ed25519Signer` built from managed key material)
        ...
        *,
        ...
        for item in checks:
            if not isinstance(item, Check):
                raise GateConfigurationError("checks must contain chit.gate.Check values")
    ...
    def execute(
        ...
        call: Callable[[], _T],
        *,
        adapter_id: str = "chit.gate.callable",
        receipt_type: EffectReceiptType = EffectReceiptType.ACKNOWLEDGED,
    ) -> GateOutcome:
