...
from chit.core.atomic_io import atomic_write_json
...
CLI_RESULT_SCHEMA_VERSION = "chit-cli-result.v1"
CLI_PROOF_RESULT_SCHEMA_VERSION = "chit-cli-proof-verification.v1"
...
def _json_object(raw: str, option: str) -> dict[str, Any]:
    try:
        ...
        raise ChitConfigError(f"{option} must be valid JSON: {exc.msg}") from exc
    if not isinstance(value, dict):
        raise ChitConfigError(f"{option} must decode to a JSON object")
    return value
...
def _parser() -> argparse.ArgumentParser:
    ...
    verify_receipt.add_argument(
        ...
        type=Path,
        required=True,
        help="path to a chit.verification-keys.v1 JSON trust-root file",
    )
    verify_receipt.add_argument(
        "--output-path",
        type=Path,
...
def _verification_keys(path: Path) -> StaticKeyProvider:
    try:
        ...
        raise ChitConfigError(f"cannot read verification keys: {exc}") from exc
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise ChitConfigError(f"verification keys must be UTF-8 JSON: {exc}") from exc
    if not isinstance(payload, dict) or set(payload) != {"schema_id", "keys"}:
        raise ChitConfigError("verification keys require exactly 'schema_id' and 'keys'")
    if payload["schema_id"] != "chit.verification-keys.v1":
        raise ChitConfigError("unsupported verification-key schema")
    raw_keys = payload["keys"]
    if not isinstance(raw_keys, list) or not raw_keys:
        raise ChitConfigError("verification keys require at least one public key")
    ...
    for index, raw_key in enumerate(raw_keys):
        if not isinstance(raw_key, dict) or set(raw_key) != {
            ...
            raise ChitConfigError(f"verification key {index} has invalid fields")
        ...
        if not all(isinstance(value, str) and value for value in (key_id, algorithm, encoded)):
            raise ChitConfigError(f"verification key {index} contains an invalid string")
        if algorithm != "ed25519":
            raise ChitConfigError(f"verification key {index} uses an unsupported algorithm")
        try:
            public_key = base64.b64decode(encoded, validate=True)
        except (ValueError, binascii.Error) as exc:
            raise ChitConfigError(f"verification key {index} contains invalid base64") from exc
        ...
        if identity in keys:
            raise ChitConfigError(f"verification key {index} duplicates a key identity")
...
def _verify_receipt(args: argparse.Namespace) -> int:
    try:
        try:
            ...
            raise ChitConfigError(f"cannot read proof bundle: {exc}") from exc
        bundle = ProofBundleV1.from_bytes(bundle_bytes)
        keys = _verification_keys(args.keys)
    except (AssuranceError, ChitConfigError) as exc:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    raise ChitConfigError(f"Unsupported command: {args.command}")
