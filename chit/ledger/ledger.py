...
from chit.core.atomic_io import _fsync_enabled
...
from chit.core.task import LedgerStats, Task, TaskPriority, TaskResult, TaskStatus
from chit.ledger.wal import GENESIS_HASH, WalHead, WalHeadStore, WalLog, WalReplay
...
class TaskLedger:
    ...
    def __init__(
        ...
        self._wal_enabled = os.getenv("CHIT_LEDGER_WAL", "1").strip() != "0"
        ...
        try:
            self._wal_compact_entries = int(
                os.getenv(
                    "CHIT_LEDGER_WAL_COMPACT_ENTRIES",
                    str(_WAL_COMPACT_ENTRIES_DEFAULT),
                )
            )
        except ValueError as exc:
            raise ChitConfigError("CHIT_LEDGER_WAL_COMPACT_ENTRIES must be an int") from exc
        if self._wal_compact_entries < 1:
            raise ChitConfigError("CHIT_LEDGER_WAL_COMPACT_ENTRIES must be positive")
    ...
    def _validate_tasks(tasks: builtins.list[Task]) -> None:
        ...
        from chit.verify.base import registry  # noqa: PLC0415
    ...
    def mark_done(self, task_id: str, result: TaskResult) -> Task:
        """VERIFYING --- DONE. Called ONLY by ChitRunner after verifier passes."""
        with self._lock:
            data = self._read_raw()
            self._assert_exists(data, task_id)
            task = Task.from_dict(data["tasks"][task_id])
    ...
    def _write_raw(self, data: dict[str, Any]) -> None:
        ...
        write on multi-hundred-task ledgers. Set ``CHIT_LEDGER_INDENT=1``
        ...
        indent = 2 if os.getenv("CHIT_LEDGER_INDENT") == "1" else None
