...
chit.core.config
---------------------------------------------------------------
ChitConfig --- central configuration for the Chit runner.
All fields have sensible defaults. Model is read from CHIT_MODEL env var
...
__all__ = ["ChitConfig", "default_data_dir"]
...
def _coerce_env_value(raw: str, hint: Any, env_key: str) -> Any:
    ...
    ``T | None`` shape used pervasively by :class:`ChitConfig`.
...
class ChitConfig:
    """
    Central configuration for ChitRunner and supporting components.
    ...
    # ------ LLM ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    model: str = field(default_factory=lambda: os.getenv("CHIT_MODEL", _DEFAULT_MODEL))
    ...
    # state by setting ``CHIT_DATA_DIR=/var/lib/chit``. Bare paths
    ...
    report_signing_key: str | None = field(
        default_factory=lambda: os.getenv("CHIT_REPORT_SIGNING_KEY"),
        repr=False,
    )
    report_signing_key_id: str = field(
        default_factory=lambda: os.getenv("CHIT_REPORT_SIGNING_KEY_ID", "operator")
    )
    ...
    def from_env(
        ...
        **overrides: Any,
        ...
        """Construct a :class:`ChitConfig` from environment variables.
        ...
        This makes ``ChitConfig`` Kubernetes ConfigMap / 12-factor
        friendly: a Deployment manifest can set
        ``CHIT_MAX_COST_USD=25.0`` and the runner
    ...
    def __post_init__(self) -> None:
        ...
        infinite-budget run.
        """
        from chit.core.exceptions import ChitConfigError  # local import

        def _require_positive(name: str, value: float) -> None:
            if value <= 0:
                raise ChitConfigError(f"ChitConfig.{name} must be > 0, got {value!r}")
        ...
        if self.temperature < 0.0:
            raise ChitConfigError(f"ChitConfig.temperature must be >= 0, got {self.temperature!r}")
        if (
            self.report_signing_key is not None
            and len(self.report_signing_key.encode("utf-8")) < 32
        ):
            raise ChitConfigError("ChitConfig.report_signing_key must contain at least 32 bytes")
        if self.report_file is not None:
            if self.report_signing_key is None:
                raise ChitConfigError(
                    "ChitConfig.report_signing_key is required when report_file is set"
                )
            if not self.report_signing_key_id:
                raise ChitConfigError("ChitConfig.report_signing_key_id must be non-empty")
        if self.report_trusted_head is not None and (
            ...
            raise ChitConfigError(
                "ChitConfig.report_trusted_head must be a lowercase SHA-256 digest"
            )
