...
from chit import __version__
from chit.core.exceptions import ChitConfigError, VerificationError
from chit.core.report import stable_hash
from chit.core.task import Task, TaskResult
from chit.verify.base import VerificationResult, VerifierRegistry, registry

PROOF_SCHEMA_VERSION = "chit-proof.v2"
...
class VerifierStep:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.verifier_id, str) or not self.verifier_id.strip():
            raise ChitConfigError("VerifierStep.verifier_id must not be empty")
        if not isinstance(self.verifier_config, dict):
            raise ChitConfigError("VerifierStep.verifier_config must be an object")
        if self.name is not None and not isinstance(self.name, str):
            raise ChitConfigError("VerifierStep.name must be a string or None")
        if not isinstance(self.blocking, bool):
            raise ChitConfigError("VerifierStep.blocking must be a boolean")
...
class VerificationContract:
    ...
    def __post_init__(self) -> None:
        if not self.contract_id.strip():
            raise ChitConfigError("VerificationContract.contract_id must not be empty")
        if not self.verifiers:
            raise ChitConfigError("VerificationContract.verifiers must contain at least one step")
...
def sign_decision(
    ...
    ``CHIT_PROOF_SIGNING_KEY``. Chit intentionally has no built-in
...
def _resolve_signing_key(signing_key: SigningKey) -> bytes:
    key = signing_key if signing_key is not None else os.getenv("CHIT_PROOF_SIGNING_KEY")
    if key is None:
        raise ChitConfigError(
            "completion proof signing requires signing_key or CHIT_PROOF_SIGNING_KEY"
        )
    encoded = key.encode("utf-8") if isinstance(key, str) else key
    if not isinstance(encoded, bytes):
        raise ChitConfigError("completion proof signing key must be text or bytes")
