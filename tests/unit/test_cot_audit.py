...
class TestGoalHijacking:
    def test_detects_goal_hijacking(self) -> None:
        ...
        violations = [
            v for v in result.violations if v.violation_type == ViolationType.GOAL_HIJACKING
        ]
...
class TestSpecContradiction:
    def test_detects_reasoning_contradicts_spec(self) -> None:
        ...
        violations = [
            v for v in result.violations if v.violation_type == ViolationType.SPEC_CONTRADICTION
        ]
        assert len(violations) >= 1
...
class TestAlignmentMirage:
    def test_detects_alignment_mirage(self) -> None:
        ...
        violations = [
            v for v in result.violations if v.violation_type == ViolationType.ALIGNMENT_MIRAGE
        ]
        assert len(violations) >= 1
