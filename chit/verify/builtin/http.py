    ...
    def __init__(
        ...
        Args:
            ...
            allow_private_targets: When ``False`` (default) the verifier
                ...
                ``CHIT_HTTP_ALLOW_PRIVATE=1``) for intentional internal
        ...
        env_allow = os.getenv("CHIT_HTTP_ALLOW_PRIVATE", "").strip() == "1"
        if not (allow_private_targets or env_allow):
            ...
            if _is_blocked_host(host):
                raise ChitConfigError(
                    ...
                    "set CHIT_HTTP_ALLOW_PRIVATE=1 to opt in."
                )
        self.url = url
