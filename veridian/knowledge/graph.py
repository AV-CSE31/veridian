...
class RegulatoryGraph:
    ...
    def add_edge(self, edge: RegEdge) -> None:
        ...
        if edge.source not in self._graph:
            raise KnowledgeGraphError(f"Source node '{edge.source}' not in graph. Add it first.")
        if edge.target not in self._graph:
            raise KnowledgeGraphError(f"Target node '{edge.target}' not in graph. Add it first.")
    ...
    def get_edges(self, node_id: str) -> list[RegEdge]:
        ...
        return [
            self._graph.edges[node_id, target]["data"] for target in self._graph.successors(node_id)
        ]
    ...
    def __repr__(self) -> str:
        return f"RegulatoryGraph(nodes={self.node_count}, edges={self.edge_count})"
