...
if TYPE_CHECKING:
    from chit.effects import EffectEventV1, EffectReceiptV1, ExecutionPermitV1


OTEL_SEMCONV_SCHEMA_V1 = "chit.otel-semconv.v1"
...
_EVENT_NAMES: Mapping[TelemetryStage, str] = {
    TelemetryStage.DECISION: "chit.assurance.decision",
    TelemetryStage.PERMIT: "chit.assurance.permit",
    TelemetryStage.EXECUTION: "chit.assurance.execution",
    TelemetryStage.RECEIPT: "chit.assurance.receipt",
}
_BASE_FIELDS = frozenset(
    {
        "chit.assurance.schema_id",
        "chit.assurance.stage",
        "chit.authorization.digest",
        "chit.decision.digest",
        "chit.decision.status",
    }
)
_PERMIT_FIELDS = frozenset(
    {
        "chit.semantic.digest",
        "chit.permit.digest",
        "chit.permit.status",
    }
)
_EXECUTION_FIELDS = frozenset(
    {
        "chit.execution.digest",
        "chit.execution.status",
    }
)
...
class AssuranceTelemetryEventV1:
    ...
    def __post_init__(self) -> None:
        ...
        if attributes["chit.assurance.schema_id"] != OTEL_SEMCONV_SCHEMA_V1:
            raise AssuranceValidationError("unsupported telemetry semantic-convention schema")
        if attributes["chit.assurance.stage"] != self.stage.value:
        ...
        decision_status = attributes["chit.decision.status"]
        ...
        if (
            ...
            and attributes["chit.permit.status"] != "issued"
        ):
        ...
        if (
            ...
            and attributes["chit.execution.status"] not in _EXECUTION_STATUSES
        ):
            raise AssuranceValidationError("unsupported execution telemetry status")
        if (
            self.stage is TelemetryStage.RECEIPT
            and attributes["chit.receipt.status"] not in _RECEIPT_STATUSES
        ):
...
class AssuranceTelemetryLinkV1:
    ...
    def event(self, stage: TelemetryStage) -> AssuranceTelemetryEventV1:
        ...
        attributes: dict[str, TelemetryAttributeValue] = {
            "chit.assurance.schema_id": OTEL_SEMCONV_SCHEMA_V1,
            "chit.assurance.stage": stage.value,
            "chit.authorization.digest": self.authorization_digest,
            "chit.decision.digest": self.decision_digest,
            "chit.decision.status": self.decision_status,
        }
        if stage in (TelemetryStage.PERMIT, TelemetryStage.EXECUTION, TelemetryStage.RECEIPT):
            ...
            attributes.update(
                {
                    "chit.semantic.digest": self.semantic_digest,
                    "chit.permit.digest": self.permit_digest,
        ...
        if stage in (TelemetryStage.EXECUTION, TelemetryStage.RECEIPT):
            ...
            attributes.update(
                {
                    "chit.execution.digest": self.execution_digest,
                    "chit.execution.status": self.execution_status,
                }
            )
        if stage is TelemetryStage.RECEIPT:
            ...
            attributes.update(
                {
                    "chit.receipt.digest": self.receipt_digest,
                    "chit.receipt.status": self.receipt_status,
                }
            )
