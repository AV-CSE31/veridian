...
class TestAttackSuite:
    ...
    def test_context_manipulation_patterns_at_least_10(self) -> None:
        ctx = [
            p for p in BUILTIN_ATTACK_PATTERNS if p.category == AttackCategory.CONTEXT_MANIPULATION
        ]
        assert len(ctx) >= 10

    def test_verification_bypass_patterns_at_least_10(self) -> None:
        byp = [
            p for p in BUILTIN_ATTACK_PATTERNS if p.category == AttackCategory.VERIFICATION_BYPASS
        ]
        assert len(byp) >= 10
...
class TestAttackScorer:
    def _make_results(self, total: int, detected: int, bypassed: int) -> list[AttackResult]:
        results = []
