    ...
    regulated_tasks = [
        {
            ...
            "evidence": {
                "passed": True,
                "verifier": "schema",
                "required_fields": ["severity", "department", "justification"],
            },
        },
        {
            "task_id": "extract_pii",
...
def run_demo() -> None:
    ...
    print("  Hash algorithm:     SHA-256 (hash-linked)")
    print("  HMAC signed:        Yes")
    print(f"  Chain integrity:    {'INTACT' if intact else 'BROKEN'}")
    print("  Standard:           EU AI Act")
    ...
    print("\n  --- Tamper Detection ---")
    print("  Modifying entry 3 (risk_assessment)...")
    ...
    print("  Verdict: Retroactive modification DETECTED mathematically.")
    print("  A compliance officer can prove logs were NOT altered post-hoc.")

    # Show report excerpt
    print("\n  --- Report Excerpt ---")
    # Regenerate with original chain for clean report
    clean_gen = ComplianceReportGenerator(proof_chain=build_auditable_pipeline())
    clean_report = clean_gen.generate(ComplianceStandard.EU_AI_ACT)
    for line in clean_report.to_markdown().split("\n")[:10]:
        print(f"  {line}")

    elapsed = int((time.monotonic() - start) * 1000)
    print(f"\n  Elapsed: {elapsed}ms")
    print("  Article 12 satisfied: logging, traceability, model binding, policy attestation")
    print("  Article 19 satisfied: automatically generated, machine-readable logs")
    print("=" * 75)
