...
class TestMemoryIntegrityPassCases:
    ...
    def test_passes_clean_memory_entry(
    ...
    def test_passes_no_memory_entries(
        self, verifier: MemoryIntegrityVerifier, task: Task
    ) -> None:
    ...
    def test_passes_empty_entries_list(
        self, verifier: MemoryIntegrityVerifier, task: Task
    ) -> None:
...
class TestMemoryIntegrityFailCases:
    ...
    def test_fails_missing_source_attribution(
        ...
        result = TaskResult(
            raw_output="done",
            structured={
                "memory_entries": [{"key": "endpoint", "value": "https://example.com"}]
            },
        )
        vr = verifier.verify(task, result)
    ...
    def test_fails_oversized_entry(
        self, verifier: MemoryIntegrityVerifier, task: Task
    ) -> None:
        ...
        result = TaskResult(
            ...
            structured={
                "memory_entries": [
                    {"key": "data", "value": "x" * 200, "source_task_id": "t0"}
                ]
            },
        )
    ...
    def test_fails_contradiction_flag(
        self, verifier: MemoryIntegrityVerifier, task: Task
    ) -> None:
...
class TestMemoryIntegrityStateless:
    ...
    def test_multiple_calls_independent(self) -> None:
        ...
        clean = TaskResult(
            ...
            structured={
                "memory_entries": [
                    {"key": "x", "value": "1", "source_task_id": "t0"}
                ]
            },
        )
        dirty = TaskResult(
