...
class BaseStorage(ABC):
    ...
    def get(self, task_id: str, *, tenant_id: str | None = None) -> Task:
        ...
        When *tenant_id* is set, the implementation must verify the task
        belongs to that tenant (e.g. task_id starts with ``<tenant>::``)
        and raise ``TenantIsolationError`` if it does not.  When *tenant_id*
        is ``None`` (the default) no filtering is applied â€” backward compatible.

        Raises:
            TaskNotFound: if no task with the given ID exists.
            TenantIsolationError: if *tenant_id* is set and the task belongs
    ...
    def list_all(self, *, tenant_id: str | None = None) -> list[Task]:
        ...
        When *tenant_id* is set, only tasks whose ID starts with
        ``<tenant_id>::`` are returned.  When ``None`` (the default),
        all tasks are returned â€” backward compatible.
        """
