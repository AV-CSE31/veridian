"""
chit.providers.litellm_provider
------------------------------------
...
# provider endpoint to hit. An attacker who can influence ``CHIT_MODEL``
...
# ``CHIT_ALLOWED_MODELS`` (comma-separated prefixes). Setting
# ``CHIT_ALLOWED_MODELS=*`` disables the guard for development.
...
def _allowed_model_prefixes() -> tuple[str, ...]:
    ...
    override = os.getenv("CHIT_ALLOWED_MODELS")
    if not override:
...
def _validate_model_string(model: str) -> None:
    ...
    Raises:
        ProviderError: when ``model`` does not match an allowed prefix and
            the allowlist is not disabled via ``CHIT_ALLOWED_MODELS=*``.
    """
    ...
    if not any(model.startswith(prefix) for prefix in prefixes):
        raise ProviderError(
            ...
            f"({sorted(prefixes)}). Set CHIT_ALLOWED_MODELS to expand the "
            "allowlist, or CHIT_ALLOWED_MODELS=* to disable the guard."
        )
...
class LiteLLMProvider(LLMProvider):
    ...
    def __init__(
        ...
        self.model: str = model or os.getenv("CHIT_MODEL") or self.DEFAULT_MODEL
