...
class TestInit:
    ...
    def test_fresh_init_is_immediately_status_compatible(self, tmp_path: Path) -> None:
        ledger_file = tmp_path / "ledger.json"
...
class TestRun:
    ...
    def test_dry_run_with_share_report(self, tmp_path: Path) -> None:
        ledger_file = _seed_ledger(
            tmp_path,
            [
                _make_task("t1", status="pending"),
        ...
        assert result.exit_code == 0
        assert "share report" in result.stdout.lower()

        report_files = list(tmp_path.glob("veridian_share_*.md"))
